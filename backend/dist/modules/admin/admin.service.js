import { findOrganizationsByCreator, createOrganization as createOrgRepo, createAdminInvite as createAdminInviteRepo, findInvitesByOrgIds, listSuperAdmins as listSuperAdminsRepo, grantSuperAdmin as grantSuperAdminRepo, revokeSuperAdmin as revokeSuperAdminRepo, } from './admin.repository.js';
...
export async function createInvite(uid, input) {
...
export async function addSuperAdmin(requestingUid, input) {
    const { email } = input;
...
export async function removeSuperAdmin(requestingUid, targetUid) {
...
export async function bootstrapSuperAdmin(input) {
    ...
    if (config.NODE_ENV === 'production') {
        try {
            ...
            const hasSuperAdmin = listUsersResult.users.some((user) => user.customClaims?.superAdmin === true);
            if (hasSuperAdmin) {
                return {
...
export async function devSetSuperAdmin(email) {
    const auth = getAuth();
    const user = await auth.getUserByEmail(email);
...
export function checkSuperAdminStatus(uid, email, claims) {

import { findChildAssignment, findMember, assignChildToSpecialist, unassignChildFromSpecialist, } from './assignments.repository.js';
export async function assignChild(orgId, input) {
...
export async function unassignChild(orgId, input) {
    const { childId } = input;
    // Verify child is assigned to org
    const childAssignment = await findChildAssignment(orgId, childId);
    if (!childAssignment || childAssignment.data?.assigned !== true) {

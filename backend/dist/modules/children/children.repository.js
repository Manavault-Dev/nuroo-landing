...
import { getOrgChildrenRef, getOrgChildRef, getChildRef, getChildProgressRef, getChildTasksRef, getChildFeedbackRef, getParentRef, getFirestore, } from '../../infrastructure/database/collections.js';
export async function findOrgChildren(orgId, role, uid) {
...
export async function getChildSummary(childId) {
    const childRef = getChildRef(childId);
    const childSnap = await childRef.get();
    let name = 'Unknown';
    let age;
    let speechStepId;
    let speechStepNumber;
    let lastActiveDate;
...
export async function getChildDetail(childId, orgId) {
    const childRef = getChildRef(childId);
    const childSnap = await childRef.get();
    if (!childSnap.exists) {
        return null;
...
export async function getChildTimeline(childId, days) {
    const now = new Date();

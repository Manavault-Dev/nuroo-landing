...
import { getFirestore, getOrgInviteRef, getParentInviteRef, getInviteRef, getOrganizationRef, getOrganizationsRef, getOrgMemberRef, getSpecialistRef, getChildRef, getOrgChildRef, getParentRef, } from '../../infrastructure/database/collections.js';
import { nowTimestamp, futureTimestamp } from '../../shared/utils/timestamp.js';
export async function createOrgInvite(orgId, role, createdBy, expiresInDays, maxUses) {
...
export async function findOrgInvite(code) {
...
export async function findParentInvite(code) {
    const normalizedCode = code.trim().toUpperCase();
    const inviteRef = getParentInviteRef(normalizedCode);
    const inviteSnap = await inviteRef.get();
    return inviteSnap.exists ? { ref: inviteRef, data: inviteSnap.data() } : null;
}
export async function incrementInviteUsage(ref) {
    try {
...
export async function createParentInvite(specialistId, orgId) {
...
export async function upsertSpecialist(uid, email, orgId, role) {
...
export async function linkChildToOrg(childId, orgId, parentUid, specialistId) {
...
export async function upsertParent(parentUid, orgId, childId) {
    ...
    if (parentSnap.exists) {
        ...
        const orgAlreadyLinked = linkedOrgs.some((o) => o.orgId === orgId);
        if (!orgAlreadyLinked) {
            linkedOrgs.push({
                orgId,
                orgName,
...
export async function findOrCreatePersonalOrg(uid, name, email) {

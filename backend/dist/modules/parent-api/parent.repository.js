import { getFirestore, getChildRef, getChildNotesRef, getOrgChildrenRef, getOrganizationRef, getSpecialistRef, getParentRef, } from '../../infrastructure/database/collections.js';
/**
 * Verify that the parent has access to this child
 */
export async function verifyParentChildAccess(childId, parentUid) {
 ...
 */
export async function getParentLinkedOrganizations(parentUid) {
    const parentRef = getParentRef(parentUid);
    const parentSnap = await parentRef.get();
    if (!parentSnap.exists) {
        return [];
 ...
 */
export async function getChildSpecialists(childId, parentUid) {
 ...
 */
export async function getChildNotesForParent(childId, parentUid) {
    // First verify parent has access
    const hasAccess = await verifyParentChildAccess(childId, parentUid);
    if (!hasAccess) {
        throw new Error('Access denied: You do not have access to this child');

import { listChildSpecialists, listChildNotes, listParentOrganizations, listParentLinkedChildren, } from './parent.service.js';
export const parentApiRoutes = async (fastify) => {
    // GET /api/parent/children/:childId/specialists - List specialists with access to child
    fastify.get('/api/parent/children/:childId/specialists', async (request, reply) => {
        if (!request.user) {
    ...
    fastify.get('/api/parent/children/:childId/notes', async (request, reply) => {
        if (!request.user) {
            return reply.code(401).send({ error: 'Unauthorized' });
        }
        const { uid: parentUid } = request.user;

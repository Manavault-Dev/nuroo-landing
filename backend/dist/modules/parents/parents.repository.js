...
export async function findParents(orgId) {
...
export async function createParent(orgId, input) {
    const parentsRef = getOrgParentsRef(orgId);
...
export async function findParent(orgId, parentId) {
...
export async function updateParent(orgId, parentId, input) {
    const parentRef = getOrgParentsRef(orgId).doc(parentId);
    const now = nowTimestamp();
    ...
    if (input.linkedChildren !== undefined)
        updateData.linkedChildren = input.linkedChildren;
...
export async function deleteParent(orgId, parentId) {
    const parentRef = getOrgParentsRef(orgId).doc(parentId);
    await parentRef.delete();
    console.log(`[PARENTS] Deleted parent contact: ${parentId} from org ${orgId}`);
}
//# sourceMappingURL=parents.repository.js.map
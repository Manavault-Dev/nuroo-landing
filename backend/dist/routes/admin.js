...
export const adminRoute = async (fastify) => {
    ...
    fastify.post('/admin/organizations', async (request, reply) => {
        await requireSuperAdmin(request, reply);
        const db = getFirestore();
        const { uid } = request.user;
    ...
    fastify.get('/admin/invites', async (request, reply) => {
        try {
            ...
            const frontendUrl = process.env.NEXT_PUBLIC_B2B_URL || 'http://localhost:3000';
            const invites = await Promise.all(limitedDocs.map(async (doc) => {
                try {
                    const data = doc.data();
    ...
    fastify.get('/admin/orgs/:orgId/specialists', async (request, reply) => {
        try {
            await requireSuperAdmin(request, reply);
            const db = getFirestore();
            const { uid } = request.user;
            ...
            console.log(`📋 [ADMIN] Member details:`, membersSnapshot.docs.map((doc) => ({
                uid: doc.id,
                role: doc.data().role,
                status: doc.data().status,
                hasStatus: !!doc.data().status,
            })));
            const allSpecialists = await Promise.all(membersSnapshot.docs.map(async (doc) => {
            ...
            console.log(`✅ [ADMIN] Listed ${specialists.length} specialists (filtered from ${allSpecialists.length} total) for org ${orgId}`);
            console.log(`📋 [ADMIN] Specialists:`, specialists.map((s) => ({ uid: s.uid, name: s.name, role: s.role, status: s.status })));
            return {
                ok: true,
                specialists,
    ...
    fastify.get('/admin/orgs/:orgId/parents', async (request, reply) => {
        try {
            await requireSuperAdmin(request, reply);
            const db = getFirestore();
            const { uid } = request.user;
            ...
            console.log(`📋 [ADMIN] Found ${parentsSnapshot1.docs.length} parents in organizations/${orgId}/parents`);
            ...
            try {
                ...
                console.log(`📋 [ADMIN] Found ${parentsSnapshot2.docs.length} parents in orgParents/${orgId}/parents`);
            }
            catch (error) {
                console.log(`⚠️ [ADMIN] Could not fetch from orgParents collection (might not exist):`, error.message);
            ...
            const parents = await Promise.all(Array.from(parentMap.values()).map(async (doc) => {
                const data = doc.data();
                ...
                try {
                    ...
                    console.log(`⚠️ [ADMIN] Could not fetch parent user ${parentUid} from Auth:`, error.message);
                }
                ...
                const childrenSnapshot = await db
                    ...
                    .get();
                console.log(`📋 [ADMIN] Found ${childrenSnapshot.docs.length} children for parent ${parentUid}`);
                // Get full child details
                const linkedChildren = await Promise.all(childrenSnapshot.docs.map(async (childDoc) => {
            ...
            console.log(`📋 [ADMIN] Parents:`, parents.map((p) => ({
                id: p.id,
                name: p.name,
                email: p.email,
    ...
    fastify.get('/admin/orgs/:orgId/children', async (request, reply) => {
        try {
            await requireSuperAdmin(request, reply);
            const db = getFirestore();
            const { uid } = request.user;
            ...
            const childrenSnapshot = await db
                ...
                .where('assigned', '==', true)
                .get();
            console.log(`📋 [ADMIN] Found ${childrenSnapshot.docs.length} child links in organizations/${orgId}/children`);

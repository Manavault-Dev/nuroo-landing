...
const COLLECTIONS = {
    ...
    CHILDREN: 'children',
    CHILD_PROGRESS: (childId) => `children/${childId}/progress/speech`,
    CHILD_TASKS: (childId) => `children/${childId}/tasks`,
    CHILD_FEEDBACK: (childId) => `children/${childId}/feedback`,
    ORG_PARENTS: (orgId) => `orgParents/${orgId}/parents`,
};
const DEFAULT_TIMELINE_DAYS = 30;
const MIN_TIMELINE_DAYS = 7;
const MAX_TIMELINE_DAYS = 90;
async function fetchAssignedChildren(db, orgId, role, uid) {
...
async function fetchChildDetails(db, childId) {
...
function groupChildrenByParent(childrenDocs) {
...
async function enrichChildrenWithDetails(db, parentMap) {
...
async function fetchParentAuthData(parentUserId) {
    try {
...
async function buildParentConnection(db, orgId, parentUserId, children) {
...
function transformChildSummary(childId, childData, progressData, completedTasksCount) {
    return {
        id: childId,
        name: childData?.name || childData?.childName || 'Unknown',
        age: childData?.age || childData?.childAge,
...
function parseTimelineDays(daysParam) {
...
function buildActivityMap(tasksDocs) {
...
function buildFeedbackMap(feedbackDocs) {
...
function buildTimelineDays(days, activityMap, feedbackMap) {
    const timelineDays = [];
    const now = new Date();
...
export const childrenRoute = async (fastify) => {
    fastify.get('/orgs/:orgId/connections', async (request, reply) => {
        try {
            ...
            const connections = await Promise.all(Array.from(parentMap.entries()).map(([parentUserId, children]) => buildParentConnection(db, orgId, parentUserId, children)));
            return {
                ok: true,
                connections,
    ...
    fastify.get('/orgs/:orgId/children', async (request, reply) => {
        try {
            const { orgId } = request.params;
            const member = await requireOrgMember(request, reply, orgId);
            const { uid } = request.user;
    ...
    fastify.get('/orgs/:orgId/children/:childId', async (request, reply) => {
        try {
    ...
    fastify.get('/orgs/:orgId/children/:childId/timeline', async (request, reply) => {
        try {
            const { orgId, childId } = request.params;
            ...
            const response = { days: timelineDays };
            return response;
        }
        catch (error) {

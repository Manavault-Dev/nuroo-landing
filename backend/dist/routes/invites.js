...
export const invitesRoute = async (fastify) => {
    fastify.post('/orgs/:orgId/invites', async (request, reply) => {
        if (!request.user) {
    ...
    fastify.post('/orgs/:orgId/parent-invites', async (request, reply) => {
        if (!request.user) {
            return reply.code(401).send({ error: 'Unauthorized' });
        }
        try {
            ...
            console.log(`✅ [INVITES] Created parent invite code ${inviteCode} for specialist ${uid} in org ${orgId}`);
            return {
                ok: true,
                inviteCode,
                expiresAt: expiresAt.toISOString(),
    ...
    fastify.post('/api/org/parent-invites/validate', async (request, reply) => {
        if (!request.user) {
            return reply.code(401).send({ error: 'Unauthorized' });
        }
        try {
    ...
    fastify.post('/api/org/parent-invites/use', async (request, reply) => {
        if (!request.user) {
            return reply.code(401).send({ error: 'Unauthorized' });
        }
        try {
    ...
    fastify.post('/api/org/parent-invites/accept', async (request, reply) => {
        ...
        try {
            ...
            const orgId = inviteData.orgId;
            const specialistId = inviteData.specialistId;
            ...
            if (specialistId) {
                ...
                console.log('⚠️  [ACCEPT] No specialistId in invite! Child will NOT be visible to specialists, only to Org Admin');
    ...
    fastify.get('/api/org/parent-connection/status', async (request, reply) => {
        ...
        try {
            ...
            console.log(`✅ [CONNECTION] Found ${connections.length} connection(s) for parent ${parentUid}`);
            return {
                ok: true,
    ...
    fastify.get('/api/specialist/connections', async (request, reply) => {
        ...
        try {
            ...
            console.log('🔍 [SPECIALIST_CONNECTIONS] Getting connections for specialist:', specialistUid, 'orgId:', orgId);
            if (!orgId) {
                ...
                for (const orgDoc of orgsSnapshot.docs) {
                    ...
                    if (memberSnap.exists) {
                        ...
                        allConnections.push(...connections.map((conn) => ({
                            ...conn,
                            orgId,
    ...
    async function getConnectionsForSpecialist(db, orgId, specialistUid) {
        ...
        const assignedChildrenSnap = await orgChildrenRef
            ...
            .get();
        console.log(`📋 [SPECIALIST_CONNECTIONS] Found ${assignedChildrenSnap.docs.length} children for specialist ${specialistUid} in org ${orgId}`);
        ...
        // Get parent details
        const connections = await Promise.all(Array.from(parentMap.entries()).map(async ([parentUserId, children]) => {
            ...
            try {
                ...
                console.log(`⚠️ [SPECIALIST_CONNECTIONS] Could not fetch parent user ${parentUserId} from Auth:`, error.message);
            }
    ...
    fastify.get('/api/parent/organizations', async (request, reply) => {
        ...
        try {
            ...
            console.log(`✅ [PARENT] Found ${organizations.length} organization(s) for parent ${parentUid}`);
            return {
                ok: true,
                organizations,

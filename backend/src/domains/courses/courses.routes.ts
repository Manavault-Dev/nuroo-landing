...
const createCourseSchema = z.object({
  ...
  visibility: z
    ...
    .default('PRIVATE')
    .transform((value) => (value === 'marketplace' ? 'PUBLIC' : value === 'org_only' ? 'PRIVATE' : value)),
  accessPolicy: z
    .enum(['FREE', 'PAID', 'VERIFIED_SPECIAL_NEEDS', 'INVITATION_ONLY'])
    .optional(),
...
export const coursesRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{ Params: { orgId: string; courseId: string; moduleId: string } }>(
    ...
    async (request, reply) => {
      ...
      const id = newId(
        db,
        `organizations/${orgId}/courses/${courseId}/modules/${moduleId}/lessons`
      )
      const ts = now()
      const lesson = {
        id,

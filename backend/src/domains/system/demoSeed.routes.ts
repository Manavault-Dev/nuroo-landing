...
const DEMO_DATA = [
  {
    child: { firstName: 'Айдар', lastName: 'Бекович', dateOfBirth: '2018-03-15', gender: 'male', diagnosis: 'ЗРР', primaryConcern: 'Задержка речевого развития' },
    parent: { fullName: 'Бекова Гүлнара', phone: '+996700000001', email: 'demo.parent1@demo.nuroo.app' },
  },
  {
    child: { firstName: 'Малика', lastName: 'Эркинова', dateOfBirth: '2017-07-22', gender: 'female', diagnosis: 'Дислалия', primaryConcern: 'Нарушение звукопроизношения' },
    parent: { fullName: 'Эркинов Бакыт', phone: '+996700000002', email: 'demo.parent2@demo.nuroo.app' },
  },
  {
    child: { firstName: 'Тимур', lastName: 'Асанов', dateOfBirth: '2019-01-10', gender: 'male', diagnosis: 'ОНР II', primaryConcern: 'Общее недоразвитие речи' },
    parent: { fullName: 'Асанова Айнура', phone: '+996700000003', email: 'demo.parent3@demo.nuroo.app' },
  },
]
...
const DEMO_TASKS = [
  {
    ...
    instructions: ['Улыбнуться и показать зубы', 'Удерживать положение 5 секунд', 'Расслабить мышцы', 'Повторить 5 раз'],
  },
  {
    ...
    instructions: ['Произнести звук «д-д-д» быстро', 'Добавить вибрацию кончика языка', 'Зафиксировать правильное положение'],
  },
  {
  ...
  },
  {
    ...
    instructions: ['Повторить слоги: са-со-су-сы', 'Произнести чётко и медленно', 'Ускорить темп постепенно'],
  },
  {
    ...
    difficulty: 'easy',
    estimatedDuration: 10,
    instructions: ['Назвать предмет на картинке', 'Выполнить действие по заданию', 'Подтвердить выполнение'],
  },
]
    ...
    async (request, reply) => {
      try {
        ...
        for (let i = 0; i < createdChildIds.length; i++) {
          ...
          groupBatch.set(
            db.doc(`specialists/${uid}/groups/${groupId}/children/${childId}`),
            { childId, addedAt: now }
          )
          groupBatch.set(
            db.doc(`specialists/${uid}/groups/${groupId}/parents/${parentUid}`),
            { childIds: [childId], addedAt: now }
          )
        }
    ...
    async (request, reply) => {
      try {
        ...
        const [orgChildrenSnap, tasksSnap, groupsSnap, orgParentsSnap] = await Promise.all([
          ...
          db.collection(`specialists/${uid}/groups`).where('orgId', '==', orgId).where('isDemo', '==', true).get(),
          db.collection(`orgParents/${orgId}/parents`).where('isDemo', '==', true).get(),
        ])

        const globalChildIds = orgChildrenSnap.docs.map((d) => d.data().childId).filter(Boolean) as string[]
        const parentUids = orgParentsSnap.docs.map((d) => d.data().parentUserId).filter(Boolean) as string[]
